...
$(document).ready(function () {
	...
	$("#setHourButton").click(function () {
		...
		$.ajax({
			...
			console.log(json);
				$.ajax({
					type: "GET",
					url: "http://localhost:8080/timelogger/workmonths",
					contentType: "application/json; charset=utf-8",
					dataType: "json",
					traditional: true,
					success: function (json) {
	...
	function drawCalendar(actualMonth) {
		...
		$.ajax({
			type: "GET",
			url: "http://localhost:8080/timelogger/workmonths/" + actualMonth.getFullYear().toString() + "/" + (actualMonth.getMonth() + 1).toString(),
			contentType: "application/json; charset=utf-8",
			dataType: "json",
			traditional: true,
			success: function (json) {
